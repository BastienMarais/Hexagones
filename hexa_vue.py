...
class MainWindow(tk.Frame):
    def __init__(self, parent, controleur, largeur, hauteur, nbcasesx, nbcasesy, unit):
        ...
        self.labels = ["Effacer Tout", "Effacer Resultats", "Aleatoire","Parcours en profondeur", "Parcours en largeur", "Composantes connexes", "Bellman-Ford", "Dijskstra", "A star"]
...
class Grille(tk.Canvas):
    ...
    def reset(self):
       for x,y in self.coord_to_hex:
            self.changeCouleurHex(x, y, "white")
       self.resetResults()
    ...
    def onLClick(self, event):
        if not self.parent.parent.freezed:
